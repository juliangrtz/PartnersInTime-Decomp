...
def render_markdown(
    ...
    lines = [
        ...
        "",
        f"A complete static pass over the {field_usage_document['member_count']} nonempty field-data members classifies {field_usage_document['valid_script_count']:,}",
        f"unique slot targets as valid VM entry points and {field_usage_document['private_target_count']:,} as deliberately opaque/private",
        f"targets. The valid graphs contain {sum(field_usage.values()):,} unique reachable commands using {len(field_usage)}",
        ...
        f"targets. It also exposes {field_usage_document.get('embedded_data_record_count', 0):,} referenced movement records as editable",
