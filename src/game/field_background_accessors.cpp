...
{
    u32 flag = background->tilemap_patches[index].set_flag;
    if (flag != 0x7fff) {
        ((u32 *)(gSaveData + 0x50))[flag >> 5] |= 1 << (flag & 31);
    } else {
        data_0205a02c[background->screen][(u32)index >> 5] |= 1 << (index % 32);
    }
    flag = background->tilemap_patches[index].clear_flag;
    if (flag != 0x7fff) {
        ((u32 *)(gSaveData + 0x50))[flag >> 5] &= ~(1 << (flag & 31));
    }
    if (background->tilemap_patches[index].tiles) {
        background->apply_tilemap_patch(index);
        background->set_scroll(0, background->scroll_x[0], background->scroll_y[0]);
        background->set_scroll(1, background->scroll_x[1], background->scroll_y[1]);
        background->set_scroll(2, background->scroll_x[2], background->scroll_y[2]);
    }
    return background->tilemap_patches[index].unknown_04;
}
}
