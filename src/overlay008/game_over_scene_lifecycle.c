...
SaveSceneTask *GameOverScene_Init(SaveSceneTask *task, u32 priority, u32 unused)
{
    ...
    task->base.vtable = &data_ov008_02077ff0;
    data_ov008_02078288 = task;
    ...
    GameOverScene_InitGeometry(task);
    ...
    if (irq)
        irq = GameOverSceneIrq_Init(irq, 8, 0, task);
    task->irq = irq;
    ...
    func_ov005_0206659c(func_ov008_02070334, 0, 1);
    task->phase = 0;
    return task;
}
