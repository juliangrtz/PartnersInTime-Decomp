...
SaveSceneTask *LoadScene_Init(SaveSceneTask *task, u32 priority, u32 unused)
{
    ...
    task->base.vtable = &data_ov008_02077f8c;
    data_ov008_02078284 = task;
    ...
    if (irq)
        irq = LoadSceneIrq_Init(irq, 8, 0, task);
    task->irq = irq;
    ...
    func_ov008_0206f588(task);
    func_02036988((vu16 *)0x04000050, 1, 12, 16, 16);
    func_ov005_0206659c(func_ov008_0206e9a0, 0, 1);
    task->phase = 0;
    return task;
}
