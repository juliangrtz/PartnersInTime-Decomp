...
void ShopScene_LoadBackgrounds(ShopSceneTask *task)
{
    ...
    if (!WORK.special_shop) {
        resource = Overlay5Archive_ReadEntry(
        ...
        if (resource)
            GameHeap_DeleteArray(resource);
        resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, data_ov009_0207e0dc[WORK.animated_background].entry, 0, 0);
        Overlay5DisplayBg_LoadCharactersLZ(DISPLAY_ENGINE_SUB, 3, resource, 0);
        if (resource)
            GameHeap_DeleteArray(resource);
        resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, data_ov009_0207e0de[WORK.animated_background].entry, 0, 0);
        Overlay5DisplayBg_LoadScreenLZ(DISPLAY_ENGINE_SUB, 3, resource, 0);
        if (resource)
            GameHeap_DeleteArray(resource);
        resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, data_ov009_0207e0e0[WORK.animated_background].entry, 0, 0);
        DC_FlushRange(resource, 512);
        ...
        if (WORK.animated_background) {
            resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, 78, 0, 0);
            Overlay5DisplayBg_LoadCharactersLZ(DISPLAY_ENGINE_SUB, 0, resource, 0);
            if (resource)
                GameHeap_DeleteArray(resource);
            resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, 79, 0, 0);
            MI_UncompressLZ16(resource, WORK.unknown_a0 + 0x16);
            if (resource)
                GameHeap_DeleteArray(resource);
            resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, 80, 0, 0);
            ((u8 *)resource)[0] = 0;
            ((u8 *)resource)[1] = 0;
        ...
        if (resource)
            GameHeap_DeleteArray(resource);
        resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, data_ov009_0207e0dc[3].entry, 0, 0);
        Overlay5DisplayBg_LoadCharactersLZ(DISPLAY_ENGINE_MAIN, 3, resource, 0);
        if (resource)
            GameHeap_DeleteArray(resource);
        resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, data_ov009_0207e0dc[3].unused[0], 0, 0);
        Overlay5DisplayBg_LoadScreenLZ(DISPLAY_ENGINE_MAIN, 3, resource, 0);
        if (resource)
            GameHeap_DeleteArray(resource);
        resource = Overlay5Archive_ReadEntry(ARCHIVE, 1, data_ov009_0207e0dc[3].unused[1], 0, 0);
        DC_FlushRange(resource, 512);
        Overlay5Display_BeginLoadBgExtPalette(DISPLAY_ENGINE_MAIN);
        Overlay5Display_LoadBgExtPalette(DISPLAY_ENGINE_MAIN, resource, 16384, 512);
        Overlay5Display_LoadBgExtPalette(DISPLAY_ENGINE_MAIN, resource, 24576, 512);
