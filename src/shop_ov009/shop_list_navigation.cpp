...
void ShopListNavigation_DrawCursor(ShopRowTask *task)
{
    ...
    u8 selection = ShopItemPanel_GetSelection(data_ov009_0207ea20);
    int x = ShopList_GetRowX(data_ov009_0207ea2c, selection) / 4096 - 68;
    int y = ShopList_GetRowY(data_ov009_0207ea2c, selection);
    model->animation_offset_x = x;
    model->animation_offset_y = y / 4096 + 6;
...
void ShopListNavigation_DrawArrow(ShopRowTask *task)
{
    ...
    int x = ShopList_GetRowX(data_ov009_0207ea2c, row) / 4096;
    int y = ShopList_GetRowY(data_ov009_0207ea2c, row);
    model->animation_offset_x = x;
...
void ShopListNavigation_Create(void)
{
    ...
    ShopRowTask *task = func_ov005_0206659c(ShopListNavigation_DrawCursor, 3, 1);
    ...
    if (ShopItemPanel_CanScroll(data_ov009_0207ea20)) {
        task = func_ov005_0206659c(ShopListNavigation_DrawArrow, 3, 1);
        task->part = 0;
