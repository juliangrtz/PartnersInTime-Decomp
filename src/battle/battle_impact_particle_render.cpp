...
extern "C" void BattleImpactParticle_Render(
    ...
    BattleSpriteTransform *saved_transform;
    ...
    saved_transform = &data_ov002_020c0cd8;
...

    {
        ...
        s16 adjusted_y =
            y - *(s16 *)(context + BATTLE_IMPACT_VIEW_Y_OFFSET);
        s16 adjusted_z = z;

        transform->x =
            (transform->x << 8) / BATTLE_IMPACT_MATRIX_ONE +
            ((s16)(x -
                   *(s16 *)(context + BATTLE_IMPACT_VIEW_X_OFFSET)) << 8);
        transform->y =
            (transform->y << 8) / BATTLE_IMPACT_MATRIX_ONE + (adjusted_y << 8);
        if (z < 0) {
            adjusted_z = 0;
        }
        transform->z =
            transform->z / BATTLE_IMPACT_MATRIX_ONE + adjusted_z;
    }
    ...
    if (particle->render_bits.size > BATTLE_IMPACT_STATIC_FRAME - 1) {
        model->animation_state &= ~0xFC0;
        model->animation_state_bits.state = BATTLE_IMPACT_STATIC_FRAME;
    } else {
        (*(u16 *)(gBattleContext + BATTLE_IMPACT_ANIMATION_FRAME_OFFSET))++;
        if (*(u16 *)(gBattleContext + BATTLE_IMPACT_ANIMATION_FRAME_OFFSET) >=
            BATTLE_IMPACT_FRAME_LIMIT) {
            *(u16 *)(gBattleContext + BATTLE_IMPACT_ANIMATION_FRAME_OFFSET) =
                BATTLE_IMPACT_FRAME_FIRST;
        }
    ...
    if (saved_transform < transform) {
        func_0202cd2c(&data_ov002_020c0cd8, transform, sizeof(*transform));
    } else {
        func_0202cc58(&data_ov002_020c0cd8, transform, sizeof(*transform));
    }
    model->animation_state =
