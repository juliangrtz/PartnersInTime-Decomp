...
149 independent units. Those units cover raw
...
images. There are 29,623 currently known relocations. `BattleActor_GetPartySlot`
at `0x02076F44` and
...
ARMv4T assembly or symbolic module parameters. Six maintained units from
autoload 0 now cover `ARM7_Main`, its ARM-to-Thumb thunk, `OS_Init`, IRQ mask
and handler registration, and `OS_IrqHandler`. They include 43 verified
autoload relocations; ARM7 now has 62 verified relocations in total. Every
linked component and the resulting NDS have zero differing bytes from the
