...
| `02065E30` | `BattleRumble_PlayRepeated` | Plays or queues a repeated Rumble Pak pattern |
| `02065EC4` | `BattleRumble_PlayTimed` | Plays or queues a timed Rumble Pak pattern used by hits and KO |
| `02065F58` | `BattleRumble_UpdateDelayed` | Counts down and dispatches a queued rumble pattern |
| `02065FCC` | `BattleRumble_FindFreeDelaySlot` | Finds one of four free rumble delay records |
...
| `02097808` | `BattleCommandMenu_Draw` | Draws the command-menu texture and its sine-animated directional frame |
...
| `02098B90` | `BattleCommandWheel_Draw` | Depth-sorts and perspective-renders the radial command entries (semantic C draft) |
...
The matching command-menu pair at `0x02097808`-`0x02097A74` reuses the same
64-byte transform and quarter-turn animation. When a new nonzero menu side is
fully hidden it loads resource `side - 1` into the embedded interface layer;
...
The adjacent command-wheel block at `0x02098B90` stores five eight-byte radial
entries. Its three update phases attach the wheel to the active adult or baby
actor, grow the radius to `0x2400`, and rotate each icon toward the selected
slot. Odd icon IDs lift and spin when selected. The reconstructed renderer
sorts translucent entries by cosine depth, projects them onto the battle view,
applies depth-dependent scale and shading, and draws a separate ground marker.
Confirmation starts a 12-frame squash-and-stretch bounce, optionally marks the
wheel for dismissal, and plays sound 9. The trigger is byte-identical C; the
readable update and renderer still have different MWCC register/stack layouts,
so exact builds retain the complete block's reference object.
...
Enemy status ID 3 is now exact linked C++ as well. It constructs a standalone
model from object-data slot 8, changes the owning enemy model's idle-frame
...
The pipeline is reconstructed as one cohesive C unit: the spawn, merge, and
update functions are 100-percent matches, while the 48-byte cleanup helper is
at 83.33 percent because MWCC folds one address addition. The unit therefore
remains on the exact assembly path for now. It merges overlapping values
through effects `0x36A` and `0x36B`, selects final actor-state variants `0x36C`
...
## Smash Eggs actor controller (2026-09-12)

Overlay 15 is Smash Eggs. A button-only selection from the checkpoint-83 Bros.
menu displayed that item, then entered `0x020C5B4C` with actor command 8 at
`+0x7C` and the same callback pointer at `+0x78`. The old menu field at
`gBattleContext + 0x11A` still contained 2; use the actor and native loader to
...
`+0x7E` is a formation index; this replay used Mario/Luigi carrying their babies,
indices 4 and 5. Broader attack sequencing and egg/item logic remain native.

The replay runs 2,110 frames, reaches all eight new routines and returns to the
battle command wheel. It verifies six support launches, six hit-window openings
and five closures, one reverse animation, both actor initializations and both
restorations. All 230 recorded function returns pass full controller, motion or
pair-record checks. Additional checks cover launch positions, model offsets,
animation frame count and reversal, hit calls and idle results. Object/model
pointers returned by helpers are observed outputs; their allocation and rendering
implementations are not independently verified by this probe.

The battle snapshot retains the controlled Petey Piranha encounter provenance
described above. No RAM edits were made during this replay. The original 104
story saves retain their hashes. Selection, attack and final screenshots were
captured, with live controller records, VRAM, palettes, OAM and display registers
during both support variants. Private evidence is
`build/runtime/eur_overlay15/evidence_final83.json`, generated by
`build/analysis/probe_ov15_actors.py`.
