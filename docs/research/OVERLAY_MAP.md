...
|---:|---|---:|---:|---|
| [0](../../src/field/) | Field and world | 193,316 / 366,712 | 52.72% | Field VM, active quad-region state persistence, region-trigger selection and script-slot starts, script-state resets and parent-context inheritance, owner/indexed entity lookup, coupled primary/shadow visibility and area-wide render snapshot capture; entity movement, support-motion carry and swept navigation bounds, movement completion dispatch, bounce/landing/shake phases and script pause/resume, collision and body overlap masks, navigation bounds restoration, surface maximum-height queries, planar animation-mode and playback-speed updates, and rendering; party actions, partner separation and follower contact restoration, locomotion categories and partner-state updates, direction-speed selection, auxiliary animation transitions, single-copy effects, trail visibility and stopping, follower-offset calculation, member bindings, follower state binding, height gates and slope distance scaling, default actions, leader palette animation, entity palette-profile switching and queued music, room resource/request allocation, camera and background layer motion, camera-shake setup and rumble selection, room transitions, time-hole arrival participant setup and party preparation and background-scroll snapshots for battle entry, touchscreen mask-erase setup/cleanup and VBlank graphics dispatch, effect animations and sprite graphics initialization, four-phase time-hole color cycles, screen wipes, HUD decimal sprite output, queued item notifications, entity pickup-effect positioning and message-window slide completion. |
| [2](../../src/battle/) | Battle engine | 188,504 / 362,436 | 52.01% | Both battle VM dispatchers; sub-screen model traversal and OAM assembly; directional and vertical motion handlers with coupled position channels, party rebound updates and partner-shadow positioning; damage and Stache-based hit-bonus rolls, healing-item badge bonuses, status, targeting and item-target menu construction/actor mapping, rewards and reward-list entry initialization/search and localized quantity labels with progressive drawing and acceleration, reward-panel and item-icon rendering, reserved scene-model cleanup, level-up border fading and drawing, UI, models, three-part effect initialization and actor bindings, effects and object-relative effect spawning, shared model-animation creation, attachment and model bindings, model stop/animation wrappers, saved-state particle render forwarding and cumulative model-copy trail drawing, attack callback installation, common-script active-model properties, attached-model transform/text queries and descriptor initialization, alternate-renderer effect tasks and VM model-source resolution, dialogue-window queries, close controls and manager teardown, sprite-grid capture controls, capture-transform initialization and scanline interrupts, render-override slot reservation, flexible-link mean positions, reversal and actor switching, chain-rig activity and reflected-hit processing, and mesh-task queries, common workspace initialization and loading, shared alternate-model resource control, resource loading, packed archive-slot selection and sub-screen sprite tile allocation/release, packed OAM transform initialization, numeric and cached-sprite OAM callbacks, delayed and repeating sound scheduling, timed/repeated Rumble Pak playback with four delay slots and immediate stop, scheduler and IRQ-task lifecycle, queues, VBlank transfer processing and node-list operations, transition task initialization, return setup, captured-screen fading and exit dispatch after the brightness countdown. |
...
| [12](../../src/attack_cannonball_ov012/) | Cannonballers | 4,564 / 12,032 | 37.93% | Party order, resource and object setup, launch scheduling, airborne motion, landing bounces, hit effects and cleanup. |
...
| [16](../../src/attack_mix_flower_ov016/) | Mix Flowers | 8,228 / 18,324 | 44.90% | Work allocation and resource setup, motion tables, paired effects, participant phases and exits, effect-slot reservations and kind transitions, resource changes, controller geometry, scaling, paired animation setup, phase updates and cleanup. |
