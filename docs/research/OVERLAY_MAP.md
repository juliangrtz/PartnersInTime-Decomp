...
|---:|---:|---|---|---|
| 0 | 366,712 B | Field/world code; the recovered entity object has two lifecycle variants and action/interaction state | `0x020A2BBC-0x020A2D04` (10 functions, 328 B) | Active; the complete field VM dispatcher is structured WIP C for all 290 slots `0x033..0x154` (23,488 of 23,492 original bytes, 95.96% fuzzy similarity, no remaining code-helper symbols); its common result/context/argument architecture, ROM-shaped entity/script/presentation/movement/field-state bitfields, embedded script-value ownership, dedicated matching-script loops, screen-register paths, original 91- and 96-entry entity-family inner tables including retained dead handlers, and exceptional physical command-family order are recovered |
| 2 | 362,436 B | Battle engine, battle BAI extension, combat UI, effects, transitions, and results | Many maintained units; see `BATTLE_MAP.md` | Active; the complete 19,168-byte battle-specific VM dispatcher is structured WIP C for all 182 slots (exact original size, 98.83% fuzzy, original stack/handler layout, retry form, wide-argument masking, ROM-shaped enemy stat fields, tag-specific item access, direct-effect coordinates, and actor-script ownership recovered, no remaining helper symbols), while linked matching work continues in coherent clusters |
...
| 14 | 13,860 B | Flower Bros. attack; the Bros. Flower runtime replay confirms projectile levels, participant throws, particles, bounce/hide states, sound and model cleanup | 29 functions / 3,932 B matching C/C++ | Active; 19 of 20 newly recovered projectile/participant helpers execute using supplied save 55; secondary throw and the remaining central dispatchers need further coverage |
...
| 21 | 5,280 B | Hammer attack; a live RAM input-window check reaches both missed and successful primary-hit paths | 20 functions / 1,736 B matching C/C++ | Typed 64-byte configuration and 28-byte attack state; primary/secondary transitions, initialization and return cleanup reconstructed; eight new transitions runtime-covered using supplied save 22 |
