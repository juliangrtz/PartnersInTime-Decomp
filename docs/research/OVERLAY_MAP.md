...
|---:|---:|---|---|---|
| 0 | 366,712 B | Field/world code; the recovered entity object has two lifecycle variants and action/interaction state | `0x020A2BBC-0x020A2D04` (10 functions, 328 B) | Active; the 23,492-byte field VM dispatcher is structured WIP C through all 123 consecutive slots `0x033..0x0AD` (11,944-byte partial build), including scripts, entity state/resources, animation, movement/orbits, roaming, effects, and enemy contact policy |
| 2 | 362,436 B | Battle engine, battle BAI extension, combat UI, effects, transitions, and results | Many maintained units; see `BATTLE_MAP.md` | Active; the complete 19,168-byte battle-specific VM dispatcher is now structured WIP C for all 182 slots (15,676-byte split-helper build, 28.84% fuzzy), while linked matching work continues in coherent clusters |
