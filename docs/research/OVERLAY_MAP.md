...
|---:|---:|---|---|---|
| 0 | 366,712 B | Field/world code; the recovered entity object has two lifecycle variants and action/interaction state | `0x020A2BBC-0x020A2D04` (10 functions, 328 B) | Active; the complete field VM dispatcher is structured WIP C for all 290 slots `0x033..0x154` (exact original size of 23,492 bytes, 93.96% fuzzy similarity, no remaining code-helper symbols); its common result/context/argument architecture, ROM-shaped entity/script/presentation/movement/field-state bitfields, dedicated matching-script loops, screen-register paths, original 91- and 96-entry entity-family inner tables including retained dead handlers, and exceptional physical command-family order are recovered |
