...
| 2 | 362,436 B | Battle engine, battle BAI extension, combat UI, effects, transitions, and results | Many maintained units; see `BATTLE_MAP.md` | Active; the complete 19,168-byte battle-specific VM dispatcher is structured WIP C for all 182 slots (current build 19,164 bytes, 96.45% fuzzy, original stack/handler layout, retry form, wide-argument masking, ROM-shaped enemy stat fields, tag-specific item access, and actor-script ownership recovered, no remaining helper symbols), while linked matching work continues in coherent clusters |
