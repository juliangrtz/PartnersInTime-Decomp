...
|---:|---:|---|---|---|
| 0 | 366,712 B | Field/world code; the recovered entity object has two lifecycle variants and action/interaction state | `0x020A2BBC-0x020A2D04` (10 functions, 328 B) | Active; the complete 23,492-byte field VM dispatcher is structured WIP C for all 290 slots `0x033..0x154` (one 22,852-byte function at 34.14% fuzzy similarity with no remaining code-helper symbols); its common result/context/argument architecture, ROM-shaped entity/script/presentation bitfields, dedicated matching-script loops, screen-register paths, and original 91- and 96-entry entity-family inner tables are recovered |
