...
| 2 | 362,436 B | Battle engine, battle BAI extension, combat UI, effects, transitions, and results | Many maintained units; see `BATTLE_MAP.md` | Active; the complete 19,168-byte battle-specific VM dispatcher is structured WIP C for all 182 slots (one 19,188-byte function, 76.13% fuzzy, no remaining helper symbols), while linked matching work continues in coherent clusters |
