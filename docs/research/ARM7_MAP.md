...
One hundred and four proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x9174` bytes of the first mixed autoload image with
...
There are 1,255 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 1,236 covering the 104 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x92E4` currently promoted
