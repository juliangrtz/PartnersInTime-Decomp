...
Twenty-six proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0xF28` bytes of the first mixed autoload image with
...
There are 195 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 176 covering the 26 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x1098` currently promoted
