...
Fifty proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x3938` bytes of the first mixed autoload image with
...
There are 535 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 516 covering the 50 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x3AA8` currently promoted
