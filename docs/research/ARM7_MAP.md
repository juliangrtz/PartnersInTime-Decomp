...
One hundred and sixteen proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0xA5A8` bytes of the first mixed autoload image with
...
There are 1,528 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 1,509 covering the 116 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0xA718` currently promoted
