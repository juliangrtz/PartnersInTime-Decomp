...
Ninety-nine proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x8968` bytes of the first mixed autoload image with
...
There are 1,189 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 1,170 covering the 99 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x8AD8` currently promoted
