...
Seventy proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x63E4` bytes of the first mixed autoload image with
...
There are 780 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 761 covering the 70 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x6554` currently promoted
