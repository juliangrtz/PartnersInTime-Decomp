...
Sixty-nine proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x6308` bytes of the first mixed autoload image with
...
There are 773 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 754 covering the 69 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x6478` currently promoted
