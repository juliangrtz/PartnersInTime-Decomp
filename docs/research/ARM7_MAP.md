...
One hundred and five proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x92D0` bytes of the first mixed autoload image with
...
There are 1,277 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 1,258 covering the 105 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x9440` currently promoted
