...
Ninety-seven proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x8554` bytes of the first mixed autoload image with
...
There are 1,123 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 1,104 covering the 97 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x86C4` currently promoted
