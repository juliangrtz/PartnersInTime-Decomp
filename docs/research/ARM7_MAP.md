...
Ninety-six proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x83B0` bytes of the first mixed autoload image with
...
There are 1,108 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 1,089 covering the 96 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x8520` currently promoted
