...
Forty-three proven autoload-0 units are now maintained ARMv4T/Thumb source:
...
Together these replace `0x2300` bytes of the first mixed autoload image with
...
There are 377 directly verified ARM call/branch/literal/data relocations: 19 in
resident startup and 358 covering the 43 maintained autoload-0 units. The
build decodes each instruction source and checks its calculated branch target,
referenced literal value, or stored pointer. All `0x2470` currently promoted
