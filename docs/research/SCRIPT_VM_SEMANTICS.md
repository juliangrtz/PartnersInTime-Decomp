...
| `0x06C` | `bind_entity_resource` | entity_selector, resource_index, animation_id, loop_count, preserve_previous | binds a 24-byte record from FEvent fixed section 2 to the entity; -1 retains selected subresources and the existing loop setting. A supplied loop_count selects finite repetition: the signed 11-bit counter decreases at the final frame, restarts while positive, then marks the animation finished. Counts zero and one both finish at the end of the current pass |
...
| `0x0B7` | `set_party_facing_direction` (legacy serialized name) | party_side, visible, instant | shows or hides the selected party's indicators, immediately or with a vertical slide; reconstructed as `FieldParty_SetIndicatorVisibility` |
...
| `0x101` | `start_time_hole_tunnel` | entity_0_or_minus_one, entity_1_or_minus_one, entity_2_or_minus_one, entity_3_or_minus_one, anchor_entity_or_minus_one, destination_room_id_or_minus_one, orbit_entities, center_x_offset, center_y_offset | starts the standalone 3D time-hole tunnel, replacing normal field rendering with resources loaded from FieldFx/FieldFxData.dat and scheduling destination_room_id when it is not -1. The render center is screen center (128,96) plus the signed offsets; orbit_entities makes the selected participants spiral around it. anchor_entity parity selects one of the two tunnel themes, while -1 retains the controller's prior theme bit. Cleanup requires the whiteout counter to reach 190 and the scene music to become inactive; use `0x105` and the scene-music fade `0x150` to satisfy those conditions. Cleanup re-enables the field renderer and restores participant state; a standalone run can still require a regular room reload to refill background tiles replaced by the tunnel |
...
| `0x105` | `begin_time_hole_tunnel_whiteout` | 0 literal args | begins the tunnel's whiteout phase by latching its fade bit. While the 3D time-hole renderer remains active, the clear color brightens over 190 whiteout frames and the tunnel palette is driven toward white after frame 90 |
