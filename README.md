...
- Every ARM9 and ARM7 runtime image now relinks as 43 components and 274
  independent, fixed-address ELF units.
- Ten named overlay-2 battle functions are maintained symbolic ARM source.
  They cover task enqueueing, actor lookup, enemy-load requests and 44-byte stat
  reads, enemy object-data loading and pointer fixups, the POW/DEF/level damage
  formula, HP/KO updates, and all-status reset. Every function matches its
  original bytes.
...
The verified pass covers 43 components, 274 section units, 133 maintained
