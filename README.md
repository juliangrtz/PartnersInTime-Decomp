...
- Every ARM9 and ARM7 runtime image now relinks as 43 components and 155
...
- ARM7 autoload 0 now has exact symbolic sources for `ARM7_Main` and the
  ...
  state references. `OS_Init`, `OS_EnableIrqMask`, `OS_SetIrqFunction`, and the
  main-loop ARM-to-Thumb thunk are symbolic as well. Thread bootstrap,
  switch-callback registration, and CPSR interrupt control are also maintained.
...
The verified pass covers 43 components, 155 section units, 16 maintained
units, and 29,638 currently known relocations with zero differing bytes. To
...
still conservatively marked as mixed code/data images; nine proven autoload-0
units are maintained source and 58 autoload relocations are mapped, but
