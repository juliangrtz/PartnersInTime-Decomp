...
- Maintained high-level source now includes the original Nitro math unit and
  ...
  one hundred and eighty-one byte-matching overlay-2 battle functions: forty-four
  ...
  `0xC1`, `0xC2`, and `0xE8`.
  Two matching grid-transition task functions initialize the six-byte animation
  state, clear the battle view offsets during rendering, and retire the task
  together with runtime flag `0x40` when its 8-by-6 DS-geometry pass finishes.
