...
- Every ARM9 and ARM7 runtime image now relinks as 43 components and 265
  independent, fixed-address ELF units.
- `BattleActor_GetPartySlot`, `BattleActor_GetById`,
  `BattleDamage_CalculateBase`, and `BattleActor_ApplyDamage` are maintained
  symbolic ARM functions. The actor resolvers use linker-resolved
  `gBattleContext` references; the damage functions expose the POW/DEF/level
  formula and HP/KO updates directly. All four match their original bytes.
...
The verified pass covers 43 components, 265 section units, 127 maintained
