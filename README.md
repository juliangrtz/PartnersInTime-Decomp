...
- Every ARM9 and ARM7 runtime image now relinks as 43 components and 179
...
- ARM7 autoload 0 now has exact symbolic sources for `ARM7_Main` and the
  ...
  maintained. Lock initialization, PXI FIFO initialization, the public PXI
  FIFO send/callback APIs, the hardware-timer tick subsystem, alarm
  initialization, and the public Game Pak initializer are symbolic too.
...
The verified pass covers 43 components, 179 section units, 34 maintained
units, and 29,765 currently known relocations with zero differing bytes. To
...
still conservatively marked as mixed code/data images; 27 proven autoload-0
units are maintained source and 185 autoload relocations are mapped, but
